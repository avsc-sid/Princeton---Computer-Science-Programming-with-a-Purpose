public class Clock {
    ...
    public Clock(String s) {
        String strFormat = "^\\d{2}:\\d{2}";
        if (!s.matches(strFormat))
            throw new IllegalArgumentException("Exception: Format is not HH:MM");
    ...
    private static void validate(int h, int m) {
        if (h < 0 || h >= HOURS_PER_DAY)
            throw new IllegalArgumentException("Exception: Hour should be between 0 and 23");
        if (m < 0 || m >= MINUTES_PER_HOUR)
            throw new IllegalArgumentException("Exception: Minute should be between 0 and 59");
    }
    ...
    public void toc(int d) {
        if (d < 0)
            throw new IllegalArgumentException("Exception: Not positive integer");
        int t = this.minute + d;
        this.minute = t % MINUTES_PER_HOUR;
        this.hour = (this.hour + t / MINUTES_PER_HOUR) % HOURS_PER_DAY;
    }
    // test client
    public static void main(String[] args) {
        Clock clock1 = new Clock(9, 35);
        ...
        clock2.toc(57);
        StdOut.println("Toc. (57) Clock 2: " + clock2.toString());
    }
}
