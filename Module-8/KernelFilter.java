...
public class KernelFilter {
    ...
    private static int convertToInt(double v) {
        int c = (int) v;
        if (c < 0) c = 0;
        if (c > 255) c = 255;
        return c;
    ...
    private static Picture kernel(Picture picture, double[][] weights) {
        ...
        for (int tcol = 0; tcol < picture.width(); tcol++) {
            for (int trow = 0; trow < picture.height(); trow++) {
                ...
                Color c = new Color(convertToInt(sumR), convertToInt(sumG), convertToInt(sumB));
    ...
    public static void main(String[] args) {
        ...
        Picture sourcePicture = new Picture(nameFile);
        Picture g = gaussian(sourcePicture);
        Picture s = sharpen(sourcePicture);
        Picture i = identity(sourcePicture);
        Picture lp = laplacian(sourcePicture);
        Picture e = emboss(sourcePicture);
        Picture m = motionBlur(sourcePicture);
